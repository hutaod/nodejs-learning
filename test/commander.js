...
class Option {
   ...
   * 使用给定的`flags`和`description`初始化一个新的`Option`
   * @param {string} flags
  ...
  constructor(flags, description) {
    this.flags = flags;
    this.required = flags.includes('<'); // 指定选项时必须提供值。A value must be supplied when the option is specified.
    this.optional = flags.includes('['); // 指定选项时，值是可选的。A value is optional when the option is specified.
 ...
 */
// module.exports 导出的 和 exports.program 都是 Command的实例
 ...
 * 从 '-m,--mixed <value>' 中解析 short(短) 和 long(长) 命令标识
 * @api private
 */
...
function _parseOptionFlags(flags) {
  ...
  // 现在使用原始的非常松散的解析来保持向后兼容性
  // 例如，它允许非计划中的`-sw, --short-word` [sic]。
  const flagParts = flags.split(/[ |,]+/); // 分割字符串，凡是中间又空格或者,都会被分割。
  // console.log(flags, flagParts)
  // flagParts 长度大于1，我们需要判断第二个参数是否是以[或者<开头的字符串，不是的话，就说明long命令标识存在
  ...
  // 添加对单独的short标志的支持，而不会显著改变解析！
  // 如果shortFlag不存在，说明第二个参数不是longFlag，其实这时候longFlag就是第一个参数
  // 如果longFlag是以-开头,则把获取的longFlag给shortFlag，longFlag赋值为undefined
  if (!shortFlag && /^-[^-]$/.test(longFlag)) { // /^-[^-]$/用于检查是合法的shortFlag
    shortFlag = longFlag;
    longFlag = undefined;
  }
  // 返回short命令标识和long命令
